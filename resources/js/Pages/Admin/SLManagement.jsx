...
import { FaUserGraduate, FaUser, FaEnvelope, FaCalendar, FaSpinner, FaArrowUp, FaArrowDown, FaUniversity, FaMapMarkerAlt, FaCheckCircle, FaExclamationCircle } from 'react-icons/fa';
import Breadcrumb from '@/Components/Breadcrumb';

export default function SLManagement({ slUsers, students }) {
    const { post, processing, errors } = useForm();
    const { flash } = usePage().props;
    const [activeView, setActiveView] = useState('sl'); // 'sl' or 'students'

    useEffect(() => {
        if (flash?.success) {
            // Reload the page after a short delay to show the message
            setTimeout(() => {
                router.reload({ only: ['slUsers', 'students'] });
            }, 1500);
        }
    }, [flash]);

    const handlePromote = (userId) => {
        if (confirm('Are you sure you want to promote this student to Student Leader?')) {
            post(route('admin.users.promote-sl', userId), {
                onSuccess: () => {
                    router.reload({ only: ['slUsers', 'students'] });
                }
            });
    ...
    const handleDemote = (userId) => {
        if (confirm('Are you sure you want to demote this Student Leader to Student?')) {
            post(route('admin.users.demote-sl', userId), {
                onSuccess: () => {
                    router.reload({ only: ['slUsers', 'students'] });
                }
            });
                ...
                <div className="relative bg-[var(--background-glass-effect)] backdrop-blur-[20px] pt-4 border-[var(--border-glass-effect)]">
                    <div className="flex items-center justify-center">
                        <div className="inline-flex bg-gray-200 rounded-lg p-1 shadow-inner">
                            <button
                                onClick={() => setActiveView('sl')}
                                className={`px-6 py-3 rounded-md font-medium transition-all duration-200 flex items-center space-x-2 ${
                                    activeView === 'sl'
                                        ? 'text-white shadow-md'
                                        : 'text-gray-600 hover:text-gray-800'
                                }`}
                                style={activeView === 'sl' ? { backgroundColor: '#fdb73e' } : {}}
                            >
                                <FaUserGraduate className="w-5 h-5" />
                                <span>Student Leaders ({slUsers.total || slUsers.data.length})</span>
                            </button>
                            <button
                ...
                <div className="relative bg-[var(--background-glass-effect)] backdrop-blur-[20px] p-6 border-[var(--border-glass-effect)]">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center space-x-3">
                            <div className="inline-flex items-center justify-center h-20 w-20 rounded-2xl" style={{ backgroundColor: 'rgba(253, 183, 62, 0.2)' }}>
                                <FaUserGraduate className="w-10 h-10" style={{ color: '#fdb73e' }} />
                            </div>
                            <div>
                                ...
                                <p className="text-[#212121]">
                                    {slUsers.total || slUsers.data.length} Student Leader{(slUsers.total || slUsers.data.length) !== 1 ? 's' : ''}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Empty State */}
                    {slUsers.data.length === 0 && (
                        <div className="text-center py-12">
                            <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: 'linear-gradient(to top right, #fdb73e, #fdb73e)' }}>
                            ...
                            <p className="text-[#212121]">
                                No Student Leaders found at the moment.
                            </p>
                        </div>
                    )}

                    {/* Pagination */}
                    {slUsers.links && slUsers.links.length > 3 && (
                        <div className="mt-6 flex justify-center">
                            <div className="flex space-x-2">
                                {slUsers.links.map((link, index) => (
                                    <button
                                        key={index}
                                        ...
                                        style={link.active ? { backgroundColor: '#fdb73e' } : {}}
